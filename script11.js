...
// | Метод     | Що робить                                                     | Повертає                                      | Приклад використання                         | Коли використовувати                                                           |
// | --------- | ------------------------------------------------------------- | --------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------ |
// | `forEach` | Перебирає масив і виконує функцію для кожного елемента        | `undefined`                                   | `[1,2,3].forEach(n => console.log(n))`       | Коли потрібен **побічний ефект** (console.log, зміна змінної, виклики функцій) |
// | `map`     | Створює новий масив, застосовуючи функцію до кожного елемента | Новий масив                                   | `let squares = [1,2,3].map(n => n*n)`        | Коли треба **отримати новий масив** на основі існуючого                        |
// | `filter`  | Створює новий масив з елементів, які проходять перевірку      | Новий масив                                   | `let even = [1,2,3,4].filter(n => n%2===0)`  | Коли треба **вибрати елементи за умовою**                                      |
// | `reduce`  | Згортання масиву до одного значення                           | Будь-яке значення (number, string, object...) | `let sum = [1,2,3].reduce((acc,n)=>acc+n,0)` | Коли треба **отримати одне значення** з масиву (сума, множення, об’єкт)        |
// | `some`    | Перевіряє, чи **хоч один** елемент відповідає умові           | `true` / `false`                              | `[1,2,3].some(n=>n>2)`                       | Коли хочеш **перевірити наявність хоча б одного елемента**                     |
// | `every`   | Перевіряє, чи **всі** елементи відповідають умові             | `true` / `false`                              | `[1,2,3].every(n=>n>0)`                      | Коли хочеш **перевірити, чи всі елементи відповідають умові**                  |
